...
#define EXTERN_MEMSET 1

#define EXTERN_TRAPAVAILABLE 0
...
#define INTERNAL_FUNCTIONS_ARE_STATIC false
...
typedef enum { fadeToColor,
  fadeToGamma } XFadeCommand;
...
#elif !defined(TrapAvailable)
internal_ Boolean TrapAvailable(short trap);
#endif
...
Get1MonitorState(GDHandle theDevice) {
  ...
  if ((**original).gChanCnt == 1) {
    ...
    SetHandleSize((Handle)hacked, sizeof(GammaTbl) + (**original).gFormulaSize + dataSize * 3);
...
XCalcFadeToColor(GammaInfoHdl from, const RGBColor* color, UInt16 whichStep, UInt16 numSteps) {
  ...
  for (chan = 0; chan < (**hacked).gChanCnt; ++chan) {
    ...
    for (i = 0; i < size; i++) {
      dstptr[i] = srcptr[i] - ((long)srcptr[i] - (long)channelDestination) * whichStep / numSteps;
    }
    srcptr += size;
    dstptr += size;
  }
...
XCalcFadeToGamma(GammaInfoHdl from, GammaInfoHdl to, UInt16 whichStep, UInt16 numSteps) {
  ...
  for (i = 0; i < size; i++) {
    dstptr[i] = src1ptr[i] - (long)(((long)src1ptr[i] - (long)src2ptr[i]) * whichStep / numSteps);
  }
...
#if !EXTERN_TRAPAVAILABLE && !defined(TrapAvailable)
internal_ Boolean
